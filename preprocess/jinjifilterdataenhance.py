...
case_name_mcmdCodeBertDiff = "./mcmdDIFFPTRNEWFFiraN13/"
case_name_mcmdCBCS = "./mcmdCBCSPTRFFiraN13/"
...
def fixDiffCamel(code_token,diff,methodNameLst,classNameLst,fieldNameLst):
    ...
    for i in lines:
        ...
        if i[0] == "+" or i[0] == "-":
            ...
            for j in lstp:
                jlow = j.lower()
                if ((is_camel_case(j) or jlow in allset)):
                    if jlow in checkMethodLst and jlow not in checkClassLst:
                        methodLst.add(j)
                    if jlow in checkClassLst:
                        classLst.add(j)
        ...
        if i[0] == "+":
            ...
            if nowline[0] == "@":
                ...
                if pos == -1:
                    ...
                    tp = nowline[1:pos:]
                if nowline[1::] not in addAnnotation:
                    addAnnotation.append(nowline[1::])
            ...
            if nowline[0] == "@":
                ...
                if pos == -1:
                    ...
                    tp = nowline[1:pos:]
                if nowline[1::] not in delAnnotation:
                    delAnnotation.append(nowline[1::])
        # https://github.com/bazelbuild/bazel/commit/94268c80000593d5f226595087f5cdfaab87ddbe
        # if "import" in i:
        #     if len(importpart) == 0:
...
def processFile(root,filename):
    ...
    for i in range(len(flst)):
        ...
        add_diff_note, del_diff_note = findDiffMultipleNote(ogdiff)

        tmpCBDiffdic = dict()
        tmpCBDiffdic["code_tokens"] = [rpname] + fixCodeToken(fixDiffCamel(fixCodeToken(ogdiff + " ".join(add_diff_note) + " ".join(del_diff_note)),diff,methodNamedct[CSlst[0][1::]].get(CSlst[1],[]),classNamedct[CSlst[0][1::]].get(CSlst[1],[]),fieldNamedct[CSlst[0][1::]].get(CSlst[1],[])))
