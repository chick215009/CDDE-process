...
case_name_mcmdCodeBertDiff = "./GPTDiffEnCase12/"
case_name_mcmdCBCS = "./GPTCSCase12/"
...
def getdct():
    ...
    fieldNamef = open(r"F:\codisumCSoutput\FieldName10.json","r")
    ...
    GPTdifflst = json.load(open(r"C:\Users\chick\Documents\WeChat Files\wxid_5n6dvrg9m7pq22\FileStorage\File\2024-04\ChatGPT使用手册+样例脚本\GPTLst.json","r"))
    GPTCamellst = json.load(open(r"C:\Users\chick\Documents\WeChat Files\wxid_5n6dvrg9m7pq22\FileStorage\File\2024-04\ChatGPT使用手册+样例脚本\GPTCamelFixLst.json","r"))
...
def findFileAndMethod(CSoutput,GPTdiff):
    filedescription = re.compile("Change in .+?java for .+?  ").findall(CSoutput)
    methodDescription = re.compile("Change in  method .+? to .+? with parameter .+?:").findall(CSoutput)
    classDescription = re.compile("Change in  class .+?:").findall(CSoutput)

    for i in filedescription:
        lst = i.split()
        if lst[2][:-5:] not in GPTdiff:
            GPTdiff = GPTdiff + i

    for i in methodDescription:
        lst = i.split()
        t = lst[4]
        if "(" in  t:
            t = lst[4][:lst[4].index("("):]
    ...
    for i in classDescription:
        lst = i.split()
        if lst[3] not in GPTdiff:
            GPTdiff = GPTdiff + i
...
def defaultoutput(diff,CSoutput):
    ...
    for i in lines:
        ...
        if i[0] == "+":
            ...
            nowline = i[1::].strip()
            LineLst.append("Del line " + nowline)
        else:
            nowline = i.strip()
...
def processFile(root,filename):
    ...
    for i in range(len(flst)):
        ...
        if len(tmplstCS) - len(add_mul_note) - len(del_mul_note) - len(diff.split()) > 50:
            overlencnt += 1
            CSflag = False
        ...
        #gptout = fixCamel(fixCodeToken(findFileAndMethod(flst[i].rstrip(),GPTdifflst[linenum])),diff,methodNamedct[CSlst[0][1::]].get(CSlst[1],[]),classNamedct[CSlst[0][1::]].get(CSlst[1],[]),fieldNamedct[CSlst[0][1::]].get(CSlst[1],[]))

        tmpCSdic = dict()
        tmpCSdic["code_tokens"] = fixCodeToken([rpname] + CScode_token + GPTCamellst[linenum].split())
        # if CSflag == True:
            # tmpCSdic["code_tokens"] = CScode_token + GPTCamellst[linenum].split()
            
        # else:
        #     tmpCSdic["code_tokens"] = fixCodeToken([rpname] + defaultoutput(diff,filteredCSOutput) + GPTCamellst[linenum].split())
        ...
        lenCS += len(tmpCSdic["code_tokens"])

        tmpCBDiffdic = dict()
        tmpCBDiffdic["code_tokens"] = [rpname] + fixCodeToken(ogdiff + GPTCamellst[linenum])
        tmpCBDiffdic["docstring_tokens"] = msg
        tmpCBDiffdic["repo"] = CSlst[0]
        tmpCBDiffdic["sha"] = CSlst[1]
        tmpCBDiffdic["fira_line"] = linenum

        lenGPT += len(tmpCBDiffdic["code_tokens"])
...
def main():
    ...
    print(lenCS / len(vislst))
    print(lenGPT / len(vislst))
